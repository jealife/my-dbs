...
import { Plus, Search, Calendar, AlertTriangle, CheckCircle, ArrowUpRight, FileText, Activity, Clock, SlidersHorizontal, UserX, Loader2, Download } from 'lucide-react'
    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between px-2">
        ...
        <div className="flex flex-wrap gap-4">
          ...
          {isStudent && (
            <button className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-amber-500 text-white font-black text-xs uppercase tracking-widest shadow-xl shadow-amber-500/30 active:scale-95 transition-all">
                        ...
                        <tr key={record.id || i} className="group hover:bg-primary/2 transition-colors cursor-pointer">
                          ...
                          <td className="px-6 py-5 text-right">
                            {record.status === 'ABSENT' ? (
                              <button className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-900 border border-(--glass-border) hover:bg-amber-500/10 transition-all text-[0.85rem] font-black opacity-60 italic group-hover:opacity-100 uppercase tracking-tighter">
                                Dépôt PDF
                              </button>
