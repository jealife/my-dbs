...
import { Plus, Search, ClipboardList, Clock, AlertCircle, CheckCircle, GraduationCap, ArrowUpRight, Trophy, FileText, Target, BookOpen, TrendingUp, Loader2, X } from 'lucide-react'
...
export function ExamModuleView() {
  ...
  const [evalForm, setEvalForm] = useState({ title: '', evaluationType: 'EXAM', courseId: '', startAt: '', endAt: '', totalPoints: 20, coefficient: 1 })
  ...
  const createEvalMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setEvalForm({ title: '', evaluationType: 'EXAM', courseId: '', startAt: '', endAt: '', totalPoints: 20, coefficient: 1 })
    },
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest opacity-50 ml-1">Coefficient</label>
                <input type="number" min={1} step={0.5} value={evalForm.coefficient}
                  onChange={e => setEvalForm({ ...evalForm, coefficient: parseFloat(e.target.value) || 1 })}
                  className="w-full mt-1 p-4 rounded-2xl glass-card border border-(--glass-border) focus:border-primary/50 text-sm font-medium outline-none bg-transparent" />
              </div>
            </div>
