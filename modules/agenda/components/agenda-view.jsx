...
import { Calendar, Clock, MapPin, User, ChevronLeft, ChevronRight, Plus, Users, LayoutList, Share2, MoreHorizontal, Loader2 } from 'lucide-react'
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between px-2">
        ...
        <div className="flex flex-wrap gap-4">
          ...
          {(isTeacher || isAdmin) && (
            <button className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-primary text-white font-black text-xs uppercase tracking-widest shadow-xl shadow-primary/30 hover:-rotate-1 active:scale-95 transition-all">
        ...
        <GlassCard className="lg:col-span-3 p-0 border-none ring-1 ring-(--glass-border) shadow-none overflow-hidden">
          ...
          <div className="grid grid-cols-7 min-h-[500px] bg-white/20 dark:bg-slate-950/20">
            {isLoading ? (
              ...
              <div className="col-span-7 flex flex-col items-center justify-center py-20 gap-4 opacity-60 text-rose-500">
                <p className="text-sm font-black italic">⚠️ Erreur : {error.message}</p>
              </div>
            ) : (
              dates.map((date, i) => {
