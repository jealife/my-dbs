...
import { CreditCard, TrendingUp, Clock, CheckCircle, AlertTriangle, Download, Plus, Receipt, DollarSign, ArrowUpRight, Wallet, Loader2 } from 'lucide-react'
...
export function FinanceModuleView() {
  ...
  const { data: invoices = [], isLoading: isLoadingInvoices, error: invoicesError } = useQuery({
    ...
    queryFn: () => isStudent && studentId
      ...
      : financeService.getInvoicesByStatus('PENDING'),
  ...
  const totalPaid = paidInvoices.reduce((acc, inv) => acc + (inv.paidAmount || inv.amount || 0), 0)
  const totalDue = invoices.reduce((acc, inv) => acc + (inv.amount || 0), 0)
                ...
                (activeTab === 'pending' ? pendingInvoices : activeTab === 'paid' ? paidInvoices : invoices).map((inv, i) => (
                  <tr key={inv.id || i} className="group hover:bg-primary/2 transition-colors cursor-pointer">
                    ...
                    <td className="px-6 py-5 text-right">
                      {inv.status === 'PAID' && (
                        <a href={`/api/v1/pdf/bulletins/${inv.id}`} target="_blank" className="p-2.5 rounded-xl hover:bg-primary/10 text-muted-foreground hover:text-primary transition-all active:scale-90 inline-block">
                          <Download className="w-4.5 h-4.5" />
                        </a>
                      )}
                    </td>
