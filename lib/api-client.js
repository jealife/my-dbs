...
apiClient.interceptors.response.use(
  ...
  (error) => {
    if (error.response && (error.response.status === 401 || (error.response.status === 403 && !Cookies.get('dbs_token')))) {
      // Automatic logout on token expiration or unauthenticated 403
