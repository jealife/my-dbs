...
export const userService = {
    ...
    for (const path of paths) {
      try {
        ...
        if (list && list.length > 0) {
          const filtered = list.filter(u => !u.role || u.role.toUpperCase() === 'STUDENT' || u.role === 'Etudiant');
          if (filtered.length > 0) {
             console.log(`[MyDBS Debug] Récupération réussie sur ${path} (${filtered.length} étudiants)`);
             return filtered;
          }
        }
      } catch (e) {
  ...
  async deleteUser(entityOrId, currRole = 'STUDENT') {
    // Legacy support ou objet complet avec userId
    const isObject = typeof entityOrId === 'object' && entityOrId !== null;
    const modelId = isObject ? entityOrId.id : entityOrId;
    const userId = isObject ? (entityOrId.userId || entityOrId.user_id) : entityOrId;

    const endpoints = [];
    
    // Si on connait le UserID exact, on tente l'effacement du compte de connexion en premier
    if (userId) {
      endpoints.push(`/users/${userId}`);
    }

    // Puis on efface le profil metier associe (STUDENT, ADMISSION, TEACHER)
    if (currRole.toUpperCase() === 'STUDENT') {
      endpoints.push(`/v1/admissions/${modelId}`);
      endpoints.push(`/students/${modelId}`);
    } else {
    ...
    // We try to delete all relevant endpoints individually
    ...
    if (!anySuccess && lastError) {
      throw lastError; // Ne lance l'erreur que si RIEN n'a fonctionne
    }
    return true;
  },
  ...
  async markAsRead(id) {
    try {
      if (id === 0) return true; // Handled by read-all usually
