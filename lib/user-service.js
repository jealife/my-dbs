...
/**
 * Génère un mot de passe temporaire sécurisé (16 caractères alphanumériques + spéciaux)
 */
...
export const userService = {
  ...
  async createUser(userData) {
    ...
    if (role === 'TEACHER') {
      ...
      // Envoi des identifiants par email (apiClient inclut withCredentials pour le cookie HttpOnly)
      try {
        await apiClient.post('/email/credentials', {
          to: userData.email,
        ...
        console.error('[MyDBS] Échec envoi email identifiants enseignant:', mailError.message);
      }
    ...
    try {
      ...
      console.error('[MyDBS] Échec envoi email identifiants:', mailError.message);
    }
  ...
  async updateUser(id, data, type = 'STUDENT') {
    ...
    try {
      ...
      console.error(`[MyDBS] Erreur de mise à jour (${t}):`, e.response?.status, e.message);
