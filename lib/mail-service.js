  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #eee; border-radius: 12px; padding: 30px; line-height: 1.6;">
      ...
      <div style="background-color: #f8fafc; border: 1px dashed #cbd5e1; border-radius: 8px; padding: 20px; margin: 25px 0;">
        ...
        <p style="margin-top: 15px;"><strong>Mot de passe :</strong> <code style="background: #e2e8f0; padding: 2px 5px; border-radius: 4px;">${password}</code></p>
      </div>

      <p>Vous pouvez vous connecter dès maintenant sur la plateforme :</p>
      <a href="http://localhost:3000/login" style="display: inline-block; background-color: #3b82f6; color: white; padding: 12px 25px; border-radius: 8px; text-decoration: none; font-weight: bold; margin: 15px 0;">Accéder à MyDBS</a>
      
      <p style="font-size: 12px; color: #94a3b8; margin-top: 30px; border-top: 1px solid #eee; pt: 15px;">
        Ceci est un message automatique, merci de ne pas y répondre.<br/>
        Si vous n'êtes pas l'auteur de cette demande, veuillez contacter le support.
      </p>
    </div>
  `;
