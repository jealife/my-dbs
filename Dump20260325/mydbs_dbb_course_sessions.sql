...
CREATE TABLE `course_sessions` (
  ...
  `mode` enum('HYBRID','ONLINE','ONSITE') NOT NULL,
  `recording_link` varchar(500) DEFAULT NULL,
  `start_at` datetime(6) NOT NULL,
  `status` enum('CANCELLED','COMPLETED','ONGOING','PLANNED','POSTPONED') NOT NULL,
  `timezone` varchar(80) NOT NULL,
  `title` varchar(180) NOT NULL,
  `course_id` bigint NOT NULL,
  `lesson_id` bigint DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `idx_course_sessions_course` (`course_id`),
  KEY `idx_course_sessions_lesson` (`lesson_id`),
  KEY `idx_course_sessions_start_end` (`start_at`,`end_at`),
