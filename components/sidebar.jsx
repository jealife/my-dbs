...
export function Sidebar({ className }) {
  ...
  // Si rôle inconnu mais utilisateur connecté → on affiche tout (fallback admin)
  const filteredNav = NAVIGATION.filter(item => {
    if (!item.roles) return false
    if (item.roles.includes('ALL')) return true
    if (!userRole) return true  // fallback: show everything if role unknown
    ...
    <aside className={cn("w-[280px] h-screen border-r border-white/5 flex flex-col p-5 z-50 shadow-[4px_0_24px_rgba(0,0,0,0.2)]", className)} style={{backgroundColor: '#301D00'}}>
      ...
      <div className="mt-4 pt-5 border-t border-gray-800 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <Link href="/profile" className="flex items-center justify-center gap-2 p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-colors text-slate-400 hover:text-white">
